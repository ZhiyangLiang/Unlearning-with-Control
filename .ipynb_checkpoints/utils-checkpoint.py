...
def create_pku_dataloader_from_dataset(tokenizer, dataset, fraction=1.0, batch_size=4):
    ...
    dataset.set_format( # 设置数据集的格式
        type="torch", columns=["input_ids", "attention_mask", "start_locs"] # torch的tensor; 包含哪些列
    )

    # Add labels and make it data loader.
    data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False) # mlm (Masked Language Modeling) 是一种语言模型训练方法, 其中输入文本的一部分token会被随机掩码, 模型需要预测这些被掩码的token
...
def get_truthfulQA_answers_plaintext(tqa_file_path="data/TruthfulQA.csv"):
    ...
    for ans_name in ans_names:
        ...
        if ans_name == "Best Answer":
            all_ans.extend(answers) # a = [1, 2, 3], a.extend([4, 5]) -> a = [1, 2, 3, 4, 5], 即把一个可迭代对象中的每个元素逐一添加到列表末尾
...
def get_rand_ans_loss(bad_batch, tokenizer, normal_ans, model, K=5, device="cuda:0"):
    ...
    for batch_idx in range(bad_input_ids.shape[0]):
        ...
        # Get question.
        question = ori_text.split("###")[1].split("Question:")[-1].strip() # f"### Question: {question}\n ### Answer: {answer}"
