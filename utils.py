...
def create_tofu_dataloader_from_dataset_onlyx(data_path, tokenizer, batch_size=4):
...
def create_tofu_dataloader_from_dataset_for_test(data_path):
    ...
    with open(data_path, 'r') as file:
        for line in file:
            ...
            # question = json.loads(line)["paraphrased_question"]
...
def create_pku_dataloader_from_dataset_for_test(dataset, batch_size=4):
    ...
    def preproccess(examples):
        ...
        for i in range(len(examples["prompt"])):
            results["bad_prompt"].append(examples["prompt"][i])
        return results
    ...
    dataset.set_format(  # 设置数据集的格式
        type="torch", columns=["bad_prompt"]
    )

    return dataset

def create_truthfulqa_dataloader_for_test(batch_size=4):
    """
